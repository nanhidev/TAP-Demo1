...
public class Sheet1Page {
    ...
    public void enterNaturalLanguageQuery(String query) {
        try {
            ...
            elementUtils.clickElement(searchButton); // Click the search button after entering the query
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void areRelevantCandidateProfilesDisplayed() {
        try {
            Assert.assertTrue("Relevant candidate profiles are not displayed!", elementUtils.isElementDisplayed(relevantProfilesDisplay));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
